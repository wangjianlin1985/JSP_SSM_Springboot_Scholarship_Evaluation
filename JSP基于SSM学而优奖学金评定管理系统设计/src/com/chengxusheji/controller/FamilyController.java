...
public class FamilyController extends BaseController {
	...
	/*用户在前台通过客户端ajax方式提交添加家庭情况信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(Family family, BindingResult br,
		...
		if(familyService.queryFamily(userObj, "").size() > 0 ) {
			message = "你已经登记过家庭情况，请直接查询更新！";
			writeJsonResponse(response, success, message);
			return ;
		}
		
		
		family.setUserObj(userObj);
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		family.setUpdateTime(sdf.format(new java.util.Date()));
	...
	public String userFontlist(@ModelAttribute("userObj") UserInfo userObj,String updateTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (updateTime == null) updateTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Family/family_userFrontquery_result"; 
	}
