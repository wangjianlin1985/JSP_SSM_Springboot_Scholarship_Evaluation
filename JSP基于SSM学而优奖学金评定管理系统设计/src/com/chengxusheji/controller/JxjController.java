...
public class JxjController extends BaseController {
	...
	public void userAdd(Jxj jxj, BindingResult br,
	...
	/*学生在前台按照查询条件分页查询奖学金申请信息*/
	@RequestMapping(value = { "/userFrontlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String userFrontlist(@ModelAttribute("jxjTypeObj") JxjType jxjTypeObj,String title,@ModelAttribute("userObj") UserInfo userObj,String fdyState,String fdyUserName,String glState,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (fdyState == null) fdyState = "";
		if (fdyUserName == null) fdyUserName = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Jxj/jxj_userFrontquery_result"; 
	}
	...
	/*ajax方式更新奖学金申请信息*/
	@RequestMapping(value = "/{jxjId}/fdyUpdate", method = RequestMethod.POST)
	public void fdyUpdate(Jxj jxj, BindingResult br,
			Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		String message = "";
    	boolean success = false;
		if (br.hasErrors()) { 
		...
		jxj.setFdyUserName(session.getAttribute("fdy").toString());
