...
public class ScoreController extends BaseController {
	...
	public void add(@Validated Score score, BindingResult br,
		...
		String user_name = score.getUserObj().getUser_name();
		Colleage colleageObj = userInfoService.getUserInfo(user_name).getColleageObj();
		score.setColleageObj(colleageObj);
		
		if(scoreService.queryScore(score.getTermObj(), score.getUserObj(), null).size() > 0) {
			message = "该学生该学期的成绩已经录入过了！";
			writeJsonResponse(response, success, message);
			return ;
		}
	...
	public String userFrontlist(@ModelAttribute("termObj") TermInfo termObj,@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("colleageObj") Colleage colleageObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Score/score_userFrontquery_result"; 
	}
